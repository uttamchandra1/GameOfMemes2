const createAndDownloadMeme = (memeTemplateUrl) => {
    ...
    img.onload = () => {
      ...
      if (liveTextDiv && memecontainer) {
        ...
        const fontSize = window.getComputedStyle(liveTextDiv, null).getPropertyValue("font-size");
  ...
  const assetPromises = selectedassets.map(({ element }) => loadAssetImage(element));
